...
test.describe("nav menu", () => {
  ...
  test("welcome slideshow is visible then fades at Artwork", async ({
    page,
  }) => {
    await gotoApp(page);

    const slides = page.locator("[data-welcome-slides]");
    await expect(slides.locator('img[src*="/welcome/slide-"]')).toHaveCount(3);
    await expect
      .poll(async () => {
        const opacity = await slides.evaluate(
          (el) => getComputedStyle(el).opacity,
        );
        return Number(opacity);
      })
    ...
    await expect
      .poll(async () => {
        const style = await slides.evaluate((el) => {
          const computed = getComputedStyle(el);
          return {
            opacity: Number(computed.opacity),
            visibility: computed.visibility,
