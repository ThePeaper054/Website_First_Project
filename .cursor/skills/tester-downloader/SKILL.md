...
- **Disposable / isolated runners only** (e.g. GitHub Actions / `CI=true`): may run lockfile restores and `--with-deps` automatically when needed.
- **Everywhere else:** **Ask for confirmation first** before **every** command that changes dependencies or `node_modules` (`npm install`, `npm ci`, `pnpm add`, `pnpm install`, `yarn add`, `yarn install`, and equivalents), and before `playwright install --with-deps`.
...
- The same gates apply to **retry** installs after a failure — do not bypass confirmation on retry outside disposable runners.
...
If both package restore and browsers are missing, install package first (after approval outside CI), then browsers.
...
**Outside disposable runners:** require user confirmation before any dependency / `node_modules` restore or install command.
**In CI / disposable runners:** lockfile restore may run automatically.
...
(Substitute the exact version/range from this repo’s `package.json`. Prefer restoring via the lockfile with `npm ci` or `npm install` when the dependency is already declared — still require confirmation outside CI.)
...
On **Linux** CI or when Linux system deps are clearly missing, run:

```bash
npm exec --no -- playwright install --with-deps
```

- **Disposable / CI runners:** may run `--with-deps` automatically.
- **Elsewhere:** ask for confirmation before `--with-deps`.
...
- If install fails, show the error, try one clear fix (e.g. approved `npm install` then local `playwright install` again — still gated outside CI), then report what still blocks.
